...
module.exports = async ({ region, lambdas, fileName }) => {
  const name = stdPath.basename(fileName, '.sa');
...
async function createApi (region, name, targetLambda) {
  const sdk = buildSDK(AWS);
  const targetName = targetLambda.functionName;
  const targetArn = targetLambda.functionArn;
  const nameTokens = targetName.split('-');
  const id = nameTokens[nameTokens.length - 1];
  const role = await createRole({
    containers: [],
    name: 'ApiGateway',
    id: id
  }, [targetName], 'apigateway.amazonaws.com');
      ...
      return getRootResourceId(apiId)
        .then(rootId => {
          return createAnyMethodIdempotent(apiId, rootId)
            .then(() => addIntegrationIdempotent(apiId, rootId, role, targetArn))
            .then(() => addResourceIdempotent(apiId, rootId, '{proxy+}'))
            .then(proxyResourceId => createAnyMethodIdempotent(apiId, proxyResourceId)
              .then(() => addIntegrationIdempotent(apiId, proxyResourceId, role, targetArn)))
        })
        .then(() => deploy(apiId))
        .then(() => {
